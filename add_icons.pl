...
foreach $file (sort @files) {
    ...
    #$ingredient{'sparkling-water'} = `grep 'category:' $file | ag -i 'soda water'` if (!$ingredient{'sparkling-water'});
    $ingredient{'soda-water'} = `grep 'category:' $file | ag -i 'soda water'`;
    $ingredient{'tonic-water'} = `grep 'category:' $file | ag -i 'tonic water'`;
    #$ingredient{'water'} = `grep 'category:' $file | ag -i 'water'` if (!$ingredient{'sparkling-water'} && !$ingredient{'soda-water'} && !$ingredient{'tonic-water'});
