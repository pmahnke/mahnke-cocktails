...
$INCLUDE{'suze'} = qq|/assets/images/ingredient-suze.svg|;
...
$INCLUDE{'banana_du_bresil'} = qq|/assets/images/ingredient-banane_du_bresil.svg|;
...
$INCLUDE{'rhum_agricole'} = qq|/assets/images/ingredient-jamacian-rum.svg|; 
...
foreach $file (sort @files) {
	...
	if (/excerpt:/) {
	    ...
	    $ingredient{'rhum_agricole'} = 1 if (/rhum agricole/i);
            $ingredient{'yellow-chartreuse'} = 1 if (/yellow chartreuse/i);
	    $ingredient{'chartreuse'} = 1 if (/chartreuse/i && !$ingredient{'yellow-chartreuse'});
